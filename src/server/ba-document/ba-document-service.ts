...
async function audit(
  ...
  await tx.auditLog.create({
    data: {
      ...
      entityType,
      entityId,
      metadataJson: governedAuditMetadata(environment.APP_ENV, metadata),
    },
  });
}

export async function createBaDocument(
  input: {
    slug: string;
    primaryJourneyContentItemId: string;
    contentJson: string;
  },
  actor: GovernedContentActor,
) {
  ...
  if (!access?.allowed)
    throw new Error("BA Document Primary Journey permission denied.");
  const content = parseBaDocumentContent(JSON.parse(input.contentJson));
  return db.$transaction(async (tx) => {
    const primary = await tx.contentItem.findUnique({
      where: { id: input.primaryJourneyContentItemId },
      select: { type: true, slug: true },
    });
    validateBaDocumentPrimaryJourney({
      contentType: "BA_DOCUMENT",
      primaryJourneyContentItemId: input.primaryJourneyContentItemId,
      parentType: primary?.type,
    });
    if (content.metadata.primaryJourneySlug !== primary?.slug)
      throw new Error(
        "BA Document Primary Journey slug does not match its database relation.",
      );
    const item = await tx.contentItem.create({
      data: {
        ...
        ownerId: actor.id,
        primaryJourneyContentItemId: input.primaryJourneyContentItemId,
        previewJson: preview(input.contentJson),
      },
    });
    ...
    await audit(tx, actor.id, "BA_DOCUMENT_CREATED", "ContentItem", item.id, {
      contentItemId: item.id,
...
export async function createBaDocumentDraftFromPublished(
  ...
  return db.$transaction(async (tx) => {
    const item = await tx.contentItem.findUnique({
      where: { id: contentItemId, type: "BA_DOCUMENT" },
      select: {
        primaryJourneyContentItemId: true,
        publishedRevision: {
          select: { id: true, contentJson: true, schemaVersion: true },
        },
        revisions: {
          where: {
    ...
    if (!item?.publishedRevision)
      throw new Error("Published BA Document revision not found.");
    const access = item.primaryJourneyContentItemId
      ? await evaluateContentAccessForUser(
          actor.id,
          item.primaryJourneyContentItemId,
          "EDIT",
        )
      : null;
    if (!access?.allowed)
      throw new Error("BA Document Primary Journey permission denied.");
    if (item.revisions[0])
      return { revision: item.revisions[0], created: false as const };
    ...
    const revision = await tx.contentRevision.create({
      data: {
        ...
        status: "DRAFT",
        schemaVersion: item.publishedRevision.schemaVersion,
    ...
    return { revision, created: true as const };
  });
}

export async function saveBaDocumentDraft(
  contentItemId: string,
  revisionId: string,
  contentJson: string,
  actor: GovernedContentActor,
  ...
  return db.$transaction(async (tx) => {
    ...
    const saved = await tx.contentRevision.updateMany({
      where: {
        id: revision.id,
        contentItemId,
        status: revision.status,
      ...
      data: { contentJson, schemaVersion: content.schemaVersion },
    });
    if (saved.count !== 1) throw new Error("BA Document save conflict.");
    const readBack = await tx.contentRevision.findUnique({
    ...
    assertContentReadBack(
      contentJson,
      readBack.contentJson,
      "BA Document save",
    );
    await audit(
      tx,
      actor.id,
      "BA_DOCUMENT_DRAFT_UPDATED",
      "ContentRevision",
      revision.id,
...
export async function submitBaDocumentRevision(
  contentItemId: string,
  revisionId: string,
  actor: GovernedContentActor,
) {
...
export async function reviewBaDocumentRevision(
  contentItemId: string,
  revisionId: string,
...
async function transitionRevision(
  ...
  revisionId: string,
  actor: GovernedContentActor,
  transition: "submit" | "changes" | "reject",
  note?: string,
) {
  assertJourneyCmsWriteEnvironment();
  return db.$transaction(async (tx) => {
    ...
    const status =
      transition === "submit"
        ...
        : transition === "changes"
          ? "CHANGES_REQUESTED"
          : "REJECTED";
    const updated = await tx.contentRevision.update({
      where: { id: revision.id },
      data:
        transition === "submit"
          ? { status, submittedAt: new Date(), reviewNote: null }
          : {
              status,
              reviewerId: actor.id,
              reviewedAt: new Date(),
    ...
    await audit(
      ...
      actor.id,
      transition === "submit"
        ? "BA_DOCUMENT_SUBMITTED_FOR_REVIEW"
        : transition === "changes"
          ? "BA_DOCUMENT_CHANGES_REQUESTED"
          : "BA_DOCUMENT_REJECTED",
      "ContentRevision",
      revision.id,
      { contentItemId, revisionId },
    );
...
export async function publishBaDocumentRevisionTransaction(
  tx: Prisma.TransactionClient,
  contentItemId: string,
  revisionId: string,
  actor: GovernedContentActor,
  ...
  const now = new Date();
  const published = await tx.contentRevision.updateMany({
    where: { id: revisionId, contentItemId, status: "IN_REVIEW" },
    data: {
      status: "PUBLISHED",
      reviewerId: actor.id,
      reviewedAt: now,
  ...
  if (published.count !== 1)
    throw new Error("BA Document revision publication conflict.");
  const pointed = await tx.contentItem.updateMany({
    where: { id: contentItemId, publishedRevisionId: item.publishedRevisionId },
    data: {
      publishedRevisionId: revisionId,
      previewJson: preview(revision.contentJson),
    },
  });
  if (pointed.count !== 1)
    throw new Error("BA Document publication pointer conflict.");
  const readBack = await tx.contentItem.findUnique({
    where: { id: contentItemId },
    select: { publishedRevision: { select: { contentJson: true } } },
  });
  if (!readBack?.publishedRevision)
    throw new Error("BA Document publication read-back failed.");
  assertContentReadBack(
    revision.contentJson,
    readBack.publishedRevision.contentJson,
    "BA Document publication",
  );
  await audit(
    tx,
    actor.id,
    "BA_DOCUMENT_PUBLISHED",
    "ContentItem",
    contentItemId,
    {
      contentItemId,
...
export function publishBaDocumentRevision(
  contentItemId: string,
  revisionId: string,
  actor: GovernedContentActor,
) {
  ...
  return db.$transaction((tx) =>
    publishBaDocumentRevisionTransaction(tx, contentItemId, revisionId, actor),
  );
}

export async function rollbackBaDocumentRevision(
  contentItemId: string,
  revisionId: string,
  actor: GovernedContentActor,
) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!item || item.publishedRevisionId === target.id)
      throw new Error("BA Document revision is already published.");
    const changed = await tx.contentItem.updateMany({
      ...
      data: {
        ...
        previewJson: preview(target.contentJson),
      },
    });
    if (changed.count !== 1)
      throw new Error("BA Document rollback pointer conflict.");
    await audit(
      tx,
      actor.id,
      "BA_DOCUMENT_ROLLED_BACK",
      "ContentItem",
      contentItemId,
      {
        contentItemId,
...
export async function setBaDocumentArchived(
  contentItemId: string,
  archived: boolean,
  actor: GovernedContentActor,
) {
  ...
  return db.$transaction(async (tx) => {
    ...
    await audit(
      tx,
      actor.id,
      archived ? "BA_DOCUMENT_ARCHIVED" : "BA_DOCUMENT_RESTORED",
      "ContentItem",
      item.id,
      { contentItemId, previousArchived: item.isArchived, archived },
    );
    return updated;
  });
}
