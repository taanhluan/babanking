...
export const BaDocumentRepository = {
  async listPublishedAuthorized(userId: string) {
    const journeyIds = await getAccessibleContentIds(userId, {
  ...
  async listContributorAuthorized(userId: string) {
    ...
    return db.contentItem.findMany({
      ...
      select: {
        ...
        updatedAt: true,
        owner: { select: { name: true } },
        primaryJourney: { select: { slug: true, previewJson: true } },
        publishedRevisionId: true,
        publishedRevision: { select: { version: true, contentJson: true } },
        revisions: {
          where: {
          ...
          take: 1,
          select: { id: true, version: true, status: true, authorId: true },
        },
      },
    });
  },
  getPublished(authorization: AuthorizedBaDocument) {
    return db.contentItem.findFirst({
      where: {
        ...
        isArchived: false,
        publishedRevisionId: { not: null },
        primaryJourneyContentItemId: authorization.primaryJourneyContentItemId,
      },
      select: {
        id: true,
        slug: true,
  ...
  getPublishedTranslation(authorization: AuthorizedBaDocument, locale: string) {
    return db.contentTranslation.findFirst({
      where: {
        contentItemId: authorization.id,
        locale,
        status: "PUBLISHED",
        publishedRevisionId: { not: null },
        contentItem: {
          primaryJourneyContentItemId:
            authorization.primaryJourneyContentItemId,
        },
      },
      select: {
        title: true,
