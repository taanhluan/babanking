export type EditorNode = {
  id: string;
  type: "module" | "section" | "subsection" | "block";
  title: string;
  moduleIndex: number;
  sectionIndex?: number;
...
export function buildEditorBreadcrumb(
  ...
  const moduleNode = nodes.find(
    (node) =>
      node.type === "module" && node.moduleIndex === selected.moduleIndex,
  );
  const section =
    selected.sectionIndex === undefined
      ? undefined
      : nodes.find(
          (node) =>
            node.type === "section" &&
            node.moduleIndex === selected.moduleIndex &&
            node.sectionIndex === selected.sectionIndex,
        );
  const items = [
    { label: journeyTitle, id: "journey" },
    ...(moduleNode ? [{ label: moduleNode.title, id: moduleNode.id }] : []),
    ...(section ? [{ label: section.title, id: section.id }] : []),
    ...(selected.type === "block"
      ? [{ label: selected.title, id: selected.id }]
      : []),
  ];
