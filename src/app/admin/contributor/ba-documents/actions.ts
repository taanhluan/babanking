...
const templateName = z.enum(['BRD_STANDARD','REQUIREMENT_SPECIFICATION_STANDARD','PROCESS_SPECIFICATION_STANDARD','DATA_SPECIFICATION_STANDARD','UAT_SPECIFICATION_STANDARD']);
const text = z.string().trim().min(1);

export async function createBaDocumentAction(formData: FormData) {
  const input = { slug: slug.parse(formData.get('slug')), primaryJourneyContentItemId: z.string().cuid().parse(formData.get('primaryJourneyContentItemId')), template: templateName.parse(formData.get('template')), documentCode: text.parse(formData.get('documentCode')), title: text.parse(formData.get('title')), summary: z.string().trim().min(30).parse(formData.get('summary')) };
  const user = await import('@/lib/auth').then(({ requireRole }) => requireRole('CONTRIBUTOR'));
  const primaryAccess = await evaluateContentAccessForUser(user.id, input.primaryJourneyContentItemId, 'EDIT');
  if (!primaryAccess?.allowed) throw new Error('Primary Journey permission denied.');
  const primaryJourney = await db.contentItem.findUnique({ where: { id: input.primaryJourneyContentItemId, type: 'BANKING_JOURNEY' }, select: { slug: true } });
  ...
  content.metadata = { ...content.metadata, documentCode: input.documentCode, title: input.title, summary: input.summary, primaryJourneySlug: primaryJourney.slug };
  await createBaDocument({ slug: input.slug, primaryJourneyContentItemId: input.primaryJourneyContentItemId, contentJson: JSON.stringify(content) }, user);
...
export async function reviewBaDocumentAction(formData: FormData) {
  const documentSlug=slug.parse(formData.get('slug')), id=revisionId.parse(formData.get('revisionId')), decision=z.enum(['changes','reject']).parse(formData.get('decision')), note=z.string().trim().min(10).parse(formData.get('reviewNote'));
  const {user,document}=await requireBaDocumentAccessBySlug(documentSlug,'REVIEW');
...
export async function publishBaDocumentAction(formData: FormData) {
  const documentSlug=slug.parse(formData.get('slug')), id=revisionId.parse(formData.get('revisionId'));
  const {user,document}=await requireBaDocumentAccessBySlug(documentSlug,'PUBLISH');
  await publishBaDocumentRevision(document.id,id,user);
  revalidatePath('/ba-documents'); revalidatePath(`/ba-documents/${documentSlug}`); revalidatePath(`/admin/contributor/ba-documents/${documentSlug}`);
}
