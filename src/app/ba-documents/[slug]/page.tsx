...
import { validateTranslationStructure } from '@/server/ba-document/ba-document-authoring';
...
export default async function Page({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ module?: string }> }) {
  ...
  const source = parseBaDocumentContent(JSON.parse(record.publishedRevision.contentJson));
  const locale = await getCurrentLocale();
  const translation = locale === 'vi' ? await BaDocumentRepository.getPublishedTranslation(document, 'vi') : null;
  if (locale === 'vi' && !translation?.publishedRevision) return <TranslationUnavailable englishPath={`/ba-documents/${slug}`}/>;
