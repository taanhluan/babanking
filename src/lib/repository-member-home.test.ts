...
describe('Member Home repository boundary',()=>{
  ...
  it('queries display metadata only for authorized legacy IDs and excludes non-member catalog content, archived, and unpublished records',async()=>{const result=await ContentRepository.getMemberHomeData('member-1','en');expect(mocks.getAccessibleContentIds).toHaveBeenCalledWith('member-1',{permission:'VIEW'});expect(mocks.contentFindMany).toHaveBeenCalledWith(expect.objectContaining({where:{id:{in:['allowed','archived','draft']},type:{notIn:['BA_DOCUMENT','CUSTOMER_SEGMENT']},isArchived:false,publishedRevisionId:{not:null}}}));expect(result.recentlyUpdated.map(item=>item.title)).toEqual(['Authorized Journey']);expect(JSON.stringify(result)).not.toContain('Unauthorized Journey');expect(JSON.stringify(result)).not.toContain('Archived Journey');expect(JSON.stringify(result)).not.toContain('Draft Journey')});
