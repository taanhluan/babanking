...
export type MemberHomeData = {
  ...
  domains: { code: string; name: string; description: string }[];
  recentlyUpdated: (ContentPreview & { updatedAt: Date })[];
};
...
function previewFrom(item: {
  ...
  return title && summary
    ? { id: item.id, type: item.type, slug: item.slug, title, summary }
    : null;
}
...
export const ContentRepository = {
  async getMemberHomeData(
    ...
    const accessibleIds = await getAccessibleContentIds(userId, {
      permission: "VIEW",
    });
    if (!accessibleIds.length)
      return { permittedTypes: [], domains: [], recentlyUpdated: [] };
    const [items, domains] = await Promise.all([
      db.contentItem.findMany({
        ...
        select: {
          ...
          slug: true,
          previewJson: true,
          publishedRevision: { select: { publishedAt: true, updatedAt: true } },
        },
      }),
      db.knowledgeScope.findMany({
        where: {
          isActive: true,
          contentMappings: { some: { contentItemId: { in: accessibleIds } } },
        },
      ...
      locale === "en"
        ? items
            .flatMap((item) => {
              ...
              const updatedAt =
                item.publishedRevision?.publishedAt ??
                item.publishedRevision?.updatedAt;
    ...
    return {
      ...
      domains: domains.map((scope) => ({
        code: scope.code,
        name: locale === "vi" ? scope.nameVi : scope.nameEn,
        description:
          locale === "vi" ? scope.descriptionVi : scope.descriptionEn,
      })),
      recentlyUpdated,
    };
  },
  ...
  async listByCategory(type: ContentType): Promise<ContentPreview[]> {
    ...
    const accessibleIds = await getAccessibleContentIds(user.id, {
      type,
      permission: "VIEW",
    });
  ...
  async getContentBySlug(
    ...
    const body =
      item?.publishedRevision && parseBody(item.publishedRevision.contentJson);
  ...
  async search() {
    ...
    if (!accessibleIds.length) return [];
    const journeyIds = await getAccessibleContentIds(user.id, {
    ...
    const [items, baDocuments] = await Promise.all([
      ...
      db.contentItem.findMany({
        where: {
          ...
          isArchived: false,
          publishedRevisionId: { not: null },
          primaryJourneyContentItemId: { in: journeyIds },
        },
        select: {
          slug: true,
    ...
    const standard = items.flatMap((item) => {
      if (item.type === "BA_DOCUMENT") return [];
      const preview = previewFrom(item);
      if (!preview) return [];
      const body =
      ...
      return [
        {
          ...
          keywords,
          context: labels[item.type],
          url: `/${routes[item.type]}/${item.slug}`,
        },
      ];
    });
    const documents = baDocuments.flatMap((item) => {
      try {
        if (!item.publishedRevision) return [];
        const content = parseBaDocumentContent(
          JSON.parse(item.publishedRevision.contentJson),
        );
        return [
          {
            type: "BA Document" as const,
            title: content.metadata.title,
            summary: content.metadata.summary,
            keywords: [
              ...(content.metadata.tags ?? []),
              ...extractBaDocumentSearchText(content),
            ],
            context: `${content.documentType} · ${item.primaryJourney?.slug ?? content.metadata.primaryJourneySlug}`,
            url: `/ba-documents/${item.slug}`,
          },
        ];
      } catch {
        return [];
