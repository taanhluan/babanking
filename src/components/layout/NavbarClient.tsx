...
type Labels = { overview:string;unlock:string;methodology:string;membership:string;faq:string;login:string;request:string;journeys:string;documents:string;practice:string;cases:string;roadmap:string;search:string;workspace:string;status:string;renewal:string;admin:string;adminConsole:string;account:string;logout:string };
export function NavbarClient({ mode, role, locale, labels, permittedTypes }: { mode:'visitor'|'limited'|'member'; role?:Role; locale:SupportedLocale; labels:Labels; permittedTypes: string[] }) {
  ...
  const premiumLinks=[[labels.journeys,'/banking-journeys','BANKING_JOURNEY'],[labels.documents,'/ba-documents','BA_DOCUMENT'],[labels.practice,'/ba-practice','BA_PRACTICE'],[labels.cases,'/case-studies','CASE_STUDY'],[labels.roadmap,'/career-roadmap','CAREER_LEVEL']].filter(([, , type])=>permittedTypes.includes(type)).map(([label,href])=>[label,href] as const);
