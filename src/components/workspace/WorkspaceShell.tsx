...
export async function WorkspaceShell({ user, children }: { user: { name: string; role: Role }; children: React.ReactNode }) {
  ...
  if (user.role === 'ADMIN') links.push({ href: '/admin/contributor', label: v?'Quản lý CMS':'CMS Management' });
  ...
  return <div className="min-h-screen overflow-x-hidden bg-bgLight"><header className="bg-navy px-4 py-4 text-white"><div className="mx-auto flex min-w-0 max-w-7xl items-center justify-between gap-4"><Link href={`/${locale}`} className="min-w-0 shrink"><BrandMark /></Link><div className="min-w-0 text-right text-sm"><p className="truncate font-semibold">{user.name}</p><p className="text-slate-400">{user.role}</p></div></div></header><div className="mx-auto grid min-w-0 max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[230px_1fr] lg:px-8"><aside className="min-w-0"><nav aria-label="Workspace navigation" className="flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 lg:flex-col">{links.map((link) => <Link key={link.href} href={getLocalizedPath(link.href,locale)} className="flex min-h-11 shrink-0 items-center rounded-xl px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">{link.label}</Link>)}<form action={logoutAction} className="shrink-0"><button className="min-h-11 w-full rounded-xl px-3 text-left text-sm font-semibold text-red-700">{v?'Đăng xuất':'Sign out'}</button></form></nav></aside><main className="min-w-0">{children}</main></div></div>;
}
