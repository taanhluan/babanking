...
export function CareerRoadmapPreview() {
  const careerLevels = ['Foundation', 'Junior BA', 'Middle BA', 'Senior BA', 'Lead BA', 'Domain leader'];
  return (
    <section className="px-4 py-14 sm:px-6 lg:px-8 lg:py-16">
      <Container>
        <div className="rounded-[20px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <ol className="mt-7 grid gap-2 sm:grid-cols-2 lg:grid-cols-6">{careerLevels.map((level, index) => <li key={level} className={`relative rounded-xl border p-3 text-sm font-semibold ${index === 2 ? 'border-goldAccent bg-goldPale text-navy' : 'border-slate-200 bg-slate-50 text-slate-600'}`}><span className="block text-xs font-medium opacity-70">{index + 1}</span><span className="mt-1 block">{level}</span>{index < careerLevels.length - 1 ? <ArrowRight aria-hidden="true" className="absolute -right-3 top-1/2 z-10 hidden h-4 w-4 -translate-y-1/2 text-slate-400 lg:block" /> : null}</li>)}</ol>
          <div className="mt-6 flex flex-wrap gap-3"><Link href="/career-roadmap" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-royalBlue px-5 py-3 text-sm font-semibold text-white">View Career Roadmap <ArrowRight className="h-4 w-4" /></Link></div>
        </div>
      </Container>
    </section>
  );
